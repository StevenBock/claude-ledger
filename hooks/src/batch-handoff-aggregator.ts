...
function isTaskHandoff(filePath: string): { planName: string; taskNum: number } | null {
...
function updateExecutorState(projectDir: string, sessionId: string, state: ExecutorState): void {
...
function parseFrontmatter(content: string): TaskHandoffMeta | null {
  const fmMatch = content.match(/^---\n([\s\S]*?)\n---/);
  if (!fmMatch) return null;

  const fm = fmMatch[1];
  const meta: Partial<TaskHandoffMeta> = {};
  ...
  if (filesMatch) {
    meta.files_modified = filesMatch[1]
      .split("\n")
  ...
  if (meta.task === undefined || meta.batch === undefined) return null;

  return meta as TaskHandoffMeta;
}

function getTaskHandoffsForBatch(
  projectDir: string,
  planName: string,
  batchNum: number,
  ...
  const handoffs: TaskHandoffMeta[] = [];
...
function extractMarkdownSection(content: string, afterFrontmatter: boolean): string {
  if (afterFrontmatter) {
