...
function extractMetadata(filePath: string, type: "ledger" | "plan"): ArtifactMetadata | null {
  if (!fs.existsSync(filePath)) return null;

  const content = fs.readFileSync(filePath, "utf-8");
  const metadata: ArtifactMetadata = { type, filePath };
...
function appendToIndex(projectDir: string, metadata: ArtifactMetadata): void {
