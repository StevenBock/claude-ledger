 ...
 * SubagentStop Hook
 *
 * Captures subagent outputs when Task tool completes.
 * - Parses transcript to identify agent type
 * - Logs to agent-log.jsonl for resumability
 * - Appends summary to continuity ledger
 */
...
function parseTranscript(transcriptPath: string): AgentInfo {
    ...
    for (const line of lines) {
      try {
        ...
        if (entry.message?.content) {
          ...
          const skillMatch = content.match(/skills\/([\w-]+)\/SKILL\.md/);
          if (skillMatch) {
  ...
  result.agentId = result.agentType
    ? `${result.agentType}-${transcriptName.slice(-8)}`
    : null;
...
function writeAgentLog(projectDir: string, agentInfo: AgentInfo): void {
...
`;
  ...
  if (agentReportsMatch) {
    ...
    if (nextSectionMatch && nextSectionMatch.index) {
      content =
        content.slice(0, nextSectionMatch.index) +
        "\n## Agent Reports\n" +
        agentReport +
        content.slice(nextSectionMatch.index);
    } else {
      content += "\n## Agent Reports\n" + agentReport;
    }
  }
