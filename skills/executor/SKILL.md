...
4. For each batch:
   - **Write batch info to executor-state.json** (MANDATORY before spawning)
   - Spawn implementer -> reviewer per task IN PARALLEL
   - Wait for batch to complete (hook auto-generates batch handoff from task handoffs)
5. Pass previous batch handoff to next batch's implementers
6. Aggregate results and report

### Batch State Management

Before spawning any batch, update `executor-state.json` with batch membership:

```bash
# Write to .claude/state/sessions/{sessionId}/executor-state.json
{
...
This enables the batch-handoff-aggregator hook to know when all tasks in a batch are complete.
...
# Note: Each task gets batch number and previous task handoff path for chaining
...
## Batch Handoff (Auto-Generated)

Batch handoffs are auto-generated by the `batch-handoff-aggregator` hook when all task handoffs for a batch are complete. The executor does NOT manually write batch handoffs.

The hook aggregates task handoff frontmatter and content into `thoughts/shared/handoffs/{plan-name}-batch-{N}.md`.

**Requirements for hook to work:**
1. Executor must write batch info to executor-state.json before spawning
2. Implementers must include YAML frontmatter in task handoffs (with batch number)
3. All tasks in a batch must complete and write handoffs
...
- **Skip batch state update** - MUST write batch info to executor-state.json before spawning
- **Write batch handoffs manually** - hook generates these automatically from task handoffs
